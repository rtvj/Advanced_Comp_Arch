     ...
     return;       
				 ...
				 for(int dy = -FILTER_RADIUS; dy <= FILTER_RADIUS; dy++) {
					...
					float Pixel = (float)(sharedMem[(dy + threadIdx.y) * blockDim.y + (threadIdx.x + dx)]);
					 sumX1 += Pixel * s_SobelMatrix[(dx + FILTER_RADIUS) * FILTER_DIAMETER + (dy+FILTER_RADIUS)];
					 sumY1 += Pixel * s_SobelMatrix[(dy + FILTER_RADIUS) * FILTER_DIAMETER + (dx+FILTER_RADIUS)];
