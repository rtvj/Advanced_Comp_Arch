...
struct entry_one_bits									// Single bit predictor 
{														// counter not needed						
    bool valid;
    bool prediction;
    UINT64 tag;
...
unsigned int BTB_History = 0;				//global history register
unsigned int History_len = 8;				//
...
VOID BTB_init()
{
  int i;
    ...
    for(i = 0; i < BTB_SIZE; i++)
    {
        ...
        BTB[i].counter = 0;
	   BTB_Table[i] = 2;					//initially initialized to 2 for weakly taken
    }
...
bool BTB_prediction(ADDRINT ins_ptr)
{
    ...
    if(BTB_Table[BTB_History & 0X00FF] > 1/*BTB[index].predictionBTB[index].counter > 1*/){				
      return true;
    } else {
      return false; 
    }
...
VOID BTB_update(ADDRINT ins_ptr, bool taken)
{
    ...
    //UINT8 counter; 
	unsigned int value;
	index = BTB_History & 0X00FF;
	value = BTB_Table[index];
	BTB_History = (BTB_History << 1)|taken;
