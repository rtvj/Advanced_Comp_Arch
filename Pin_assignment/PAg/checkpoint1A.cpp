...
bool BTB_prediction(ADDRINT ins_ptr)
{
    ...
    index = mask & ins_ptr;
    UINT64 newindex;
	newindex = BTB[index].history & mask;
	prediction = BTB_Table[newindex];
    if(prediction > 1/*BTB[index].counter > 1*/){				
      return true;
    } else {
      return false; 
    }
...
VOID BTB_update(ADDRINT ins_ptr, bool taken)
{
	...
	counter = BTB_Table[(BTB[index].history & mask)];
    if (taken) {
		++counter;
		if(counter>3)
		counter = 3;
	...
	BTB_Table[(BTB[index].history & mask)] = counter;
	BTB[index].history = (BTB[index].history<<1)|taken;
