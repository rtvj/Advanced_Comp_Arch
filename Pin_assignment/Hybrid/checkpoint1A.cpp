...
struct entry_one_bits									// Hybrid predictor as suggested by McFarling 
{														
    bool valid;
    bool prediction;
    UINT64 tag;
    ...
    UINT8 counter;
	//UINT8 set;
	UINT64 History;											// using two bit predictor and PAg predictor
	UINT8 selector;										//selector has values from 0-3 to select the specific branch predictor
} BTB[BTB_SIZE];			

unsigned int history_len = 12;							//length of the history reg
...
{
    UINT64 index, table_index;
	UINT8 prediction;

    index = mask & ins_ptr;
	table_index = BTB[index].History & 0x0FFF;
    
    if(BTB[index].selector > 1/*BTB[index].counter > 1*/){				
      //return true;
	  prediction = BTB_Table[table_index];
    } else {
      //return false;
	  prediction = BTB[index].counter;	  
    }
	if(prediction > 1)
	return true;
	else
...
{
    UINT64 index, table_index;
    UINT8 Pagcount, twobitcount; 

    index = mask & ins_ptr;
	table_index = BTB[index].History & mask;
	Pagcount = BTB_Table[table_index];
	twobitcount = BTB[index].counter;
	
    //counter = BTB[index].counter;

    if (taken) {
		//BTB[index].prediction = true;
      /*counter ++;
      if (counter > 3) {
	counter = 3;
      }
	  */
	  if(Pagcount > 1)									//if PAg predictor predicts correctly and twobit predictor 
		if(twobitcount < 2)								//predicts wrongly then increment selector
			if(BTB[index].selector < 3)
				++BTB[index].selector;
	
	if(Pagcount < 2)									//if PAg predicts wrongly and twobit predicts correctly 
		if(twobitcount>1)								//then decrement the selector		
			if(BTB[index].selector > 0)
				--BTB[index].selector;
	...
	counter --;
	  ...
	  */
	//  BTB[index].prediction = false;
	if(Pagcount < 2)									//if PAg predictor predicts correctly and twobit predictor 
		if(twobitcount > 1)								//predicts wrongly then increment selector
			if(BTB[index].selector < 3)
				++BTB[index].selector;
	
	if(Pagcount > 1)									//if PAg predicts wrongly and twobit predicts correctly 
		if(twobitcount < 2)								//then decrement the selector		
			if(BTB[index].selector > 0)
				--BTB[index].selector;
    }
	...
	BTB_Table[table_index] = Pagcount;
	BTB[index].counter = twobitcount;
...
VOID WriteResults(bool limit_reached)
{
	...
	out << "percentage correct: " << ((float)CountCorrect/CountSeen)*100 << endl;
    out << "percentage missed: " << ((float)CountMissed/CountSeen)*100 << endl;

    out << "BTB size is : " <<  BTB_SIZE << endl;
    for(i = 0; i < BTB_SIZE; i++)
    {
